...
class MLLogger:
    def __init__(self, filepath="/tmp/perf.log", default_stack_offset=2):
...
mllogger = MLLogger(filepath="output.txt")
...
class MetricsLogger(Logger):
    ...
    def log_metrics(self, metrics, step):
        if self.train_loss_key in metrics:
            mllogger.event(
                key="train_loss_update",
                value=metrics[self.train_loss_key],
                metadata={
                    constants.SAMPLES_COUNT: step * self.gbs,
                },
            )
